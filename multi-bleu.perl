...
while(<STDIN>) {
    ...
    foreach my $reference (@{$REF[$s]}) {
        # deal with multiple referece, but we usually use only one reference
        $ref_sen = $reference;
#        print "$s $_ <=> $reference\n";
    ...
    for (my $n = 1; $n <= 4; $n++) {
        ...
        foreach my $ngram (keys %T_NGRAM) {
            ...
            $SENTOTAL[$n] += $T_NGRAM{$ngram};
            if (defined($REF_NGRAM{$ngram})) {
                if ($REF_NGRAM{$ngram} >= $T_NGRAM{$ngram}) {
                    $CORRECT[$n] += $T_NGRAM{$ngram};
                    $SENCORRECT[$n] += $T_NGRAM{$ngram};
                    # $corr =  $T_NGRAM{$ngram};
                    #	    print "$i e correct1 $T_NGRAM{$ngram}<BR>\n";
                }
                else {
                    $CORRECT[$n] += $REF_NGRAM{$ngram};
                    $SENCORRECT[$n] += $REF_NGRAM{$ngram};
    ...
    my @bleu = ();
    for (my $n = 1; $n <= 4; $n++) {
        if (defined ($SENTOTAL[$n]) && defined ($SENCORRECT[$n])) {
#            print STDERR "SENCORRECT[$n]:$SENCORRECT[$n] SENTOTAL[$n]:$SENTOTAL[$n]\n";
            $bleu[$n] = ($SENTOTAL[$n]) ? $SENCORRECT[$n] / $SENTOTAL[$n] : 0;
        } else {
            $bleu[$n] = 0;
        }
    }
    ...
    if ($length_translation_this_sentence < $closest_length) {
        $brevity_penalty = exp(1 - $closest_length / $length_translation_this_sentence);
    }
    my @log_bleu = ();
    $bleu = $brevity_penalty * exp((my_log($bleu[1]) + my_log($bleu[2]) + my_log($bleu[3]) + my_log($bleu[4])) / 4);
    printf "%.2f\t", 100 * $bleu;
    print "line_num:$s\t$hypo_sen <==> $ref_sen\t";
...
#printf "=                                  Total BLEU                                     =\n";
#printf "===================================================================================\n";
printf "-1============= Document BLEU = %.2f, %.1f/%.1f/%.1f/%.1f (BP=%.3f, ratio=%.3f, hyp_len=%d, ref_len=%d)=============\n",
    100 * $bleu,
    100 * $bleu[1],
    100 * $bleu[2],
    100 * $bleu[3],
